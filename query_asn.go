...
type IP struct {
	ASNum     uint32 `json:"as_num" #:"ASN自治系统号"`
	IP        string `json:"ip" #:"ip"`
	IpVersion string `json:"ip_version" #:"IP版本"`
	BGPPrefix string `json:"bgp_prefix" #:"CIDR地址"`
	Country   string `json:"country" #:"所属国家(地区)"`
	Registry  string `json:"registry" #:"ASN注册商"`
	Allocated string `json:"allocated" #:"注册日期"`
	ASName    string `json:"as_name" #:"ASN主体"`
}

func (r *IP) String() string {
...
// reverseaddr copied from miekg/dns, modified to not add the "in-addr.arpa"
// or "ip6.arpa" strings.
func reverseaddr(addr string) (string, error) {
	...
	for i := len(ip) - 1; i >= 0; i-- {
		...
		// Only append a trailing "." if this isn't the final hextet
		if i > 0 {
			buf = append(buf, '.')
		}
	}
...
func LookupIP(ip string) (IP, error) {
	...
	origin.IpVersion = ipVerson
	// Add the requested IP to the output to mimic the WHOIS output
	origin.IP = ip
	// Query the AS number to get the AS name, again to mimic the WHOIS output
